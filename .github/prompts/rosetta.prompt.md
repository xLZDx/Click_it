...
## Loop
1. Prepare  - classify + tier the request; load repo context.
2. Research - read the real files, cite file:line, no guessing.
...
5. Validate - build/run, logs clean; full restart before re-test; verify before claiming done.

## Review (opt-in)
Route to the operator's specialist agents (python-reviewer, silent-failure-hunter, ...);
use /agent-consensus for architectural plans. Empiricism over poetry: cross-check every
finding against the cited line.
